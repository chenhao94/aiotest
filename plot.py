...
for wid, w in enumerate(workload):
    for fid, f in enumerate(files):
        ...
        for t in xrange(1,5):
            for r in rw:
                ...
                for l in lib:
                    ...
                    m = re.search("(?P<time>\w+\.\w+) s in total, (?P<lock>\w+\.\w+) s holding lock, (?P<avgio>\w+\.\w+) s avg true io time, (?P<avgwait>\w+\.\w+) s avg wait time.*?(?P<iops>\w+\.\w+) iops$", res)
                    if m is None:
                    ...
                    if l == 'PosixAIO':
                       ...
                       y['avg_waittime'].append(avgwait / float(m.group('time'))) 
        ...
        p += ax2.plot(xpos, y['avg_waittime'], 'c*-', label = 'avg wait time ratio')
        ax2.legend(legend + p, tuple(lib + ['locked ratio', 'avg io ratio', 'avg wait ratio']), loc='upper center', ncol=2, bbox_to_anchor=(0.2, 1.10))
